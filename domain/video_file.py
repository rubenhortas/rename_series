...
class VideoFile(File):
    ...
    def __init__(self, files_path, file_name, testing, debugging):
        ...
        if self.debugging:
            ...
            Messages.debug_msg(
                "\tself.f_absolute_original_path: {0}".format(self.f_abs_original_path))
            Messages.debug_msg("\tself.file_name: {0}".format(self.file_name))
            Messages.debug_msg(
                "\tis well formated: {0}".format(self.__is_well_formated()))
    ...
    def __get_episode(self):
        ...
        if this_episode.episode_in_name:
            ...
            if self.debugging:
                ...
                Messages.debug_msg(
                    "\tself.episode_in_name: {0}".format(self.episode_in_name))
                Messages.debug_msg("\tself.episode: {0}".format(self.episode))
                Messages.debug_msg(
                    "\tself.new_file_name: {0}".format(self.new_file_name))
    ...
    def __expand_show_name(self):
        ...
        if self.show_name.lower() in EXPANDED_NAMES:
            ...
            if self.debugging:
                ...
                Messages.debug_msg(
                    "\texpanded_name: {0}".format(self.show_name))
        else:
            if self.debugging:
                Messages.debug_msg(
                    "{0} will be not expanded".format(self.show_name))
            ...
            if title_match:
                ...
                if episode_title != "":
                    ...
                    if self.debugging:
                        ...
                        Messages.debug_msg(
                            "\tself.episode_title: {0}".format(self.episode_title))
