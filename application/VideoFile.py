...
class VideoFile(File):
    ...
    def __init__(self, files_path, file_name, testing, debugging):
        ...
        if self.debugging:
            ...
            Messages.debug_msg('+\tself.f_absolute_original_path: %s'
                               % self.f_abs_original_path)
            Messages.debug_msg('+\tself.file_name: %s' % self.file_name)
            Messages.debug_msg('+\tis well formated: %s'
                               % self.__is_well_formated())
    ...
    def __is_well_formated(self):
        ...
        pattern = '^[\w \(\)]*[\d]{1,2}x[\d]{1,2}'
        compiled_pattern = re.compile(pattern, re.UNICODE)

        if compiled_pattern.match(self.file_name):
            return True
        else:
            return False
    ...
    def __get_episode(self):
        ...
        if this_episode.episode_in_name:
            ...
            if self.debugging:
                ...
                Messages.debug_msg('\tself.episode_in_name: %s'
                                   % self.episode_in_name)
                Messages.debug_msg('\tself.episode: %s'
                                   % self.episode)
                Messages.debug_msg('\tself.new_file_name: %s'
                                   % self.new_file_name)
    ...
    def __expand_show_name(self):
        ...
        if self.show_name in expanded_names:
            ...
            if self.debugging:
                ...
                Messages.debug_msg('\texpanded_name: %s' % self.show_name)
        else:
            if self.debugging:
                Messages.debug_msg('%s will be not expanded' % self.show_name)

    def __get_episode_title(self):

            title_pattern = re.compile('[\w ]*', re.UNICODE)
            ...
            if show_name:
                title_match = title_pattern.search(show_name)
                ...
                if title_match:
                    ...
                    if episode_title != '':
                        ...
                        if self.debugging:
                            ...
                            Messages.debug_msg('\tself.episode_title: %s'
                                               % self.episode_title)
